  ...
  return (
    <>
        <section className="pb-24">
          <div className="container px-[15px] m-auto max-w-[1430px]">
            <header className="mb-8 flex justify-between items-center">
              <h2 className="text-[1.875rem]">Featured Categories</h2>
              <Link to="#" className="text-base flex items-center">
                All Categories
            ...
            <ul className="flex flex-wrap -mx-[15px]">
              {
                categories.map((item) => {
                      return (
                        <>
                          <li key={item.id} className="product-category hover:cursor-pointer px-[15px] list-item grow-0 shrink-0 basis-1/5 max-w-[20%]">
                            <Link to={`${item.link}`} className="group-hover:pointer-events-auto text-dark">
                              <div className={`product-category-inner block box-border rounded-xl px-10 py-8 ${item.bgcolor} hover:bg-gray-100`}>
